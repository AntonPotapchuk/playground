...
def make_model(actions=6, input_shape=(constants.BOARD_SIZE, constants.BOARD_SIZE, 17)):
    ...
    probs = Dense(actions, activation='softmax')(x)
    reward = Dense(128, activation='relu', name='reward_1')(x)
...
def runner(id, model_file, shared_buffer, fifo, finished, _args):
    ...
    while not finished.value:
        ...
        fifo.put((
                 trace, reward, rewards, agent_id, agent.hit_probs, agent.avg_lengths, len(agent.tree), agent.entropies,
                 agent.iters_sec))
