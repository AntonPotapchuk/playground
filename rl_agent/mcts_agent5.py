...
class MCTSAgent(BaseAgent):
    ...
    def rollout(self, shared_buffer, finished):
        ...
        while not done and not finished.value:
            ...
            pi = self.search(root, args.mcts_iters, temp)
...
def trainer(num_episodes, fifos, shared_buffer, model, memory, writer):
    callbacks = [EarlyStopping(monitor='loss', min_delta=0.01, patience=5, verbose=1, mode='auto')]
    while num_episodes < args.num_episodes:
        while True:
            ...
            try:
                ...
                trace, reward, rewards, agent_id, hit_probs, avg_lengths, tree_size, entropies = fifo.get(timeout=args.queue_timeout)
